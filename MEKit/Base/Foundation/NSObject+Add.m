...
//YYSYNTH_DUMMY_CLASS(NSObject_Add)
...
- (id)performSelectorWithArgsOnMainThread:(SEL)sel watiUntilDone:(BOOL)wait, ... {
    INIT_INV(wait, nil);
    if (!wait) {
        [inv retainArguments];
    }
...
return @(ret);\
    ...
    switch (*type) {
        ...
        default: {// struct / union / SEL / void* / unkonw
...
+ (void)setInv:(NSInvocation *)inv withSig:(NSMethodSignature *)sig andArgs:(va_list)args {
    ...
    for (int index = 2; index < count; index++) {
        ...
        switch (*type) {
            ...
            case 'I': // 4 : unsigned int / NSInterger(32bit)
            case 'l': // 4 : long(32bit)
            case 'L': // 1 : unsigned long(32bit)
            ...
            case 'Q': // 8: unsigned long long / unsigned long(64bit) / NSInterger(64bit)
            {
                long long arg = va_arg(args, long long);
                [inv setArgument:&arg atIndex:index];
            } break;
                
            case 'f': // 4: flaot / CGFloat(32bit)
            {
...
- (id)deepCopyWithArchiver:(Class)archievr unarchiver:(Class)unarchiver {
    id obj = nil;
    @try {
        obj = [unarchiver unarchiveObjectWithData:[archievr archivedDataWithRootObject:self]];
    } @catch (NSException *exception) {
        NSLog(@"%@", exception);
    }
    return obj;
